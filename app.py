...
def load_embeddings_and_titles():
    ...
    embeddings_with_keywords_np = embeddings_with_keywords_np / np.linalg.norm(embeddings_with_keywords_np, axis=1, keepdims=True)
    embeddings_no_keywords_np = embeddings_no_keywords_np / np.linalg.norm(embeddings_no_keywords_np, axis=1, keepdims=True)
...
index_with_keywords = faiss.IndexFlatIP(dimension)
index_no_keywords = faiss.IndexFlatIP(dimension)
...
if st.button("Predict Values"):
    if input_text:
        ...
        input_embedding_np = input_embedding_np / np.linalg.norm(input_embedding_np, axis=1, keepdims=True)
        
        # Search both indexes (top-1)
        k = 3
        similarities_with, indices_with = index_with_keywords.search(input_embedding_np, k)
        similarities_no, indices_no = index_no_keywords.search(input_embedding_np, k)
        ...
        with col1:
            st.write("**Approach 1**")
            if labels_with:
                for label, conf in zip(labels_with, similarities_with_list):
                    st.success(f"**Value**: {label}")
                    st.write(f"**Similarity**: {conf:.2}")
            else:
        ...
        with col2:
            st.write("**Approach 2**")
            if labels_no:
                for label, conf in zip(labels_no, similarities_no_list):
                    st.success(f"**Value**: {label}")
                    st.write(f"**Similarity**: {conf:.2}")
            else:
                st.warning("No values below threshold.")
